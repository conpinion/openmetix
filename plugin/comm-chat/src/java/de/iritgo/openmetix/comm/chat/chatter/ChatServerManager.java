	...
	extends ChatManager
{
		...
		chatterProcessor.doProcessor (
			new Processor()
			{
				public AbstractAction getAction (User user, User newUser)
				{
					...
					UserJoinAction action =
						new UserJoinAction(
							newUser.getName (), newUser.getUniqueId (), chatChannel.getName ());
	...
	private void notifyUserAboutAllUsers (final ChatChannel chatChannel, Long userId)
	{
		...
		final ClientTransceiver clientTransceiver =
			new ClientTransceiver(newUser.getNetworkChannel ());
		...
		chatterProcessor.doProcessor (
			new Processor()
			{
				public AbstractAction getAction (User user, User newUser)
				{
					UserJoinAction action =
						new UserJoinAction(
							user.getName (), user.getUniqueId (), chatChannel.getName ());
		...
		chatterProcessor.doProcessor (
			new Processor()
			{
				public AbstractAction getAction (User user, User newUser)
				{
					...
					UserLeaveAction action =
						new UserLeaveAction(
							newUser.getName (), chatChannel.getName ().hashCode (),
							newUser.getUniqueId ());
	...
	private void notifyMembersMessage (
		final String message, final ChatChannel chatChannel, final Long userId)
	{
		ChatterProcessor chatterProcessor = new ChatterProcessor(userId, chatChannel);
				...
				public AbstractAction getAction (User user, User newUser)
				{
					...
					ChatMessageAction action =
						new ChatMessageAction(
							message, chatChannel.getChannelId (), userId.longValue (),
							newUser.getName ());
