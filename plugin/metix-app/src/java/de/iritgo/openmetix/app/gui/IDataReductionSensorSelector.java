...
/**
 * @version $Id: IDataReductionSensorSelector.java,v 1.1 2005/04/24 18:10:44 grappendorf Exp $
 */
public class IDataReductionSensorSelector
	extends JPanel
{
	/** Gaging stations. */
	...
	public JCheckBox deleteAllParameters;
	private boolean deleteAllParametersSelected;

	/**
	 * Create a new IDataReductionSensorSelector.
	 */
	public IDataReductionSensorSelector ()
	{
		...
		try
		{
			JPanel panel =
				(JPanel) new SwingEngine(this).render (
					getClass ().getResource ("/swixml/IDataReductionSensorSelector.xml"));
			...
			deleteAllParameters.addItemListener (
				new ItemListener()
				{
					public void itemStateChanged (ItemEvent e)
					{
						if (deleteAllParametersSelected == false)
						{
							deleteAllParametersSelected = true;
							parameters.setEnabled (false);
						}
						else
						{
							deleteAllParametersSelected = false;
		...
		{
			Log.logError ("client", "IDataReductionSensorSelector.initGUI", x.toString ());
		}
	}

	public boolean getDeleteAllParameters ()
	{
		return this.deleteAllParametersSelected;
	}
	...
	*/
	...
	protected void updateParameterModel ()
	{
		...
		if (station != null)
		{
			IObjectList list = station.getSensors ();
