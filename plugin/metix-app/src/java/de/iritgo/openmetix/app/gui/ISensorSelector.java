...
*
* @version $Id: ISensorSelector.java,v 1.1 2005/04/24 18:10:44 grappendorf Exp $
*/
public class ISensorSelector
	extends JPanel
{
	/** Gaging stations. */
	...
	/** Gaging sensors. */
	public JComboBox needleParameters;
	...
	/** Gaging sensor model. */
	private IObjectComboBoxModel needleParametersModel;
	...
	/** The id of the currently selected needle sensor. */
	private long selectedNeedleSensorId;

	/**
	 * Create a new ISensorSelector.
	 */
	public ISensorSelector ()
	{
		super(new GridBagLayout());

		selectedStationId = -1;
		selectedSensorId = -1;
		selectedNeedleSensorId = -1;

		try
		{
			JPanel panel =
				(JPanel) new SwingEngine(this).render (
					getClass ().getResource ("/swixml/ISensorSelector.xml"));
			...
			needleParametersModel = new IObjectComboBoxModel();
			needleParameters.setModel (needleParametersModel);
			...
			needleParameters.addItemListener (
				new ItemListener()
				{
					public void itemStateChanged (ItemEvent e)
					{
		...
		{
			Log.logError ("client", "ISensorSelector.initGUI", x.toString ());
		}
	}
	...
	*/
	...
	/**
	 * Get the id of the selected needle sensor.
	 *
	 * @return The id of the selected neelde sensor.
	 */
	public long getSelectedNeedleSensorId ()
	{
		GagingSensor sensor = (GagingSensor) needleParametersModel.getSelectedItem ();
	...
	/**
	 * Get the selected needle sensor.
	 *
	 * @return The selected sensor.
	 */
	public GagingSensor getSelectedNeedleSensor ()
	{
		return (GagingSensor) needleParametersModel.getSelectedItem ();
	}

	/**
	 * Set the station and sensorid.
	 */
	public void setStationIdSensorId (long stationId, long sensorId, long needleSensorID)
	{
		if (stationId == 0 || sensorId == 0 || needleSensorID == 0)
		{
			return;
		}
		...
		selectedNeedleSensorId = needleSensorID;
	 ...
	 * Set the sensorid.
	 */
	public void setSensorId (long sensorId, long needleSonsorId)
	{
		try
		{
			...
			if (station != null)
			{
				setStationIdSensorId (station.getUniqueId (), sensorId, needleSonsorId);
			}
			else
			{
				stations.setSelectedItem (null);
	 ...
	 * Fill the parameter with the sensors of the selected station.
	 */
	protected void updateNeedleParameterModel ()
	{
		GagingStation station = (GagingStation) stationsModel.getSelectedItem ();

		if (station != null)
		{
			needleParametersModel.update (station.getSensors ());

			for (Iterator i = station.sensorIterator (); i.hasNext ();)
			{
				GagingSensor sensor = (GagingSensor) i.next ();

				if (sensor.getUniqueId () == selectedNeedleSensorId)
				{
					needleParameters.setSelectedItem (sensor);
		...
		{
			needleParametersModel.removeAllElements ();
		}
	}
	...
	public void setEnabled (boolean enabled)
	{
		...
		needleParameters.setEnabled (enabled);
	}
	 ...
	 * @param enabled If true, the selector will be enabled.
	 */
	public void setNeedlePrametersEnabled (boolean enabled)
	{
		super.setEnabled (enabled);
		needleParameters.setEnabled (enabled);
	}
