 ...
 */
public class MeasurementRecordDialog
		...
		try
		{
			...
			JPanel panel =
				(JPanel) swingEngine.render (
					getClass ().getResource ("/swixml/MeasurementRecordDialog.xml"));
		...
		new AbstractAction()
		{
			public void actionPerformed (ActionEvent e)
			{
				...
				if (stationId != 0 && sensorId != 0)
				{
					...
					SendMeasurementServerAction action =
						new SendMeasurementServerAction(
							new Timestamp(System.currentTimeMillis ()),
							NumberTools.toDouble (value.getText (), 0.0), stationId, sensorId);
