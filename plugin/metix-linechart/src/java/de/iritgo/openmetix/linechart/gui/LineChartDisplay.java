	...
	implements InstrumentDisplay
{
	...
	public static void configureChart (
		JFreeChart chart, LineChart lineChart, LineChartDisplay lineChartDisplay)
	{
		...
		switch (lineChart.getDomainAxisMode ())
		{
			...
			case LineChart.MODE_LASTTIME:
			{
				axis.setAutoRange (false);
				axis.setRange (
					System.currentTimeMillis () - lineChart.getHistoryCount (),
	...
	public void configureSensor (LineChartSensor sensor, LineChart lineChart)
	{
		...
		Log.logInfo (
			"client", "LineChartDisplay.configureSensor",
			"Ratio for sensor <" + sensor.getSensorId () + ">: " + ratio);
		...
		final HistoricalDataRequestAction historyDataServerAction =
			new HistoricalDataRequestAction(
				lineChart.getUniqueId (), sensor.getStationId (), sensor.getSensorId (),
		...
		if (instumentUniqueId == lineChart.getUniqueId ())
		{
			...
			for (Iterator iter = sensorSeriesEntrys.iterator (); iter.hasNext ();)
			{
				...
				if (entry.sensorId == sensorId &&
					lineChart.isValidTimeStamp (timestamp.getTime ()))
				{
