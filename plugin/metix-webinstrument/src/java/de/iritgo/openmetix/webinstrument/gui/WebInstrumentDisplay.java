 ...
 */
public class WebInstrumentDisplay
	extends InstrumentGUIPane
	implements InstrumentDisplay, ActionListener
{
	/** HTML display. */
			...
			CommandTools.performAsync (
				new Command()
				{
					public void perform ()
					{
						try
						{
							html.getDocument ().putProperty (
								Document.StreamDescriptionProperty, null);

							String urlSpecTmp = webInstrument.getUrl ();

							if (
								! urlSpecTmp.startsWith ("http://") &&
								! urlSpecTmp.startsWith ("https://"))
							{
