	...
	extends BaseObject
{
	...
	public NetworkService (
		ThreadService threadBase, NetworkActionProcessorInterface reveiveNetworkActionProcessor,
	 ...
	 */
	synchronized public void callReceiveNetworkActionProcessor (
		Action action, final Channel channel)
	{
		reveiveNetworkActionProcessor.perform (
			action, new ClientTransceiver(channel.getChannelNumber (), channel));
	}
	...
	public double connect (String name, int port, ConnectObserver connectObserver)
	{
		...
		{
			Log.logError (
				"network", "NetworkService.connect",
				"Error while connecting to " + name + ":" + port);
	...
	public void closeChannel (double channelNumber)
	{
		try
		{
			synchronized (channelList)
			{
				Channel connectedChannel =
					(Channel) channelList.get (String.valueOf (channelNumber));
				...
				if (connectedChannel != null)
				{
					...
					Log.logInfo (
						"network", "NetworkService.closeChannel", "Closing channel " +
						channelNumber);
				}
			}
		}
		...
		{
			Log.logError (
				"network", "NetworkService.closeChannel",
				"Error while closing channel " + channelNumber + ": " + x.toString ());
		}
	}
	...
	public void closeAllChannel ()
	{
		...
		catch (Exception x)
		{
			Log.logError (
				"network", "NetworkService.closeChannel",
				"Error while closing channel :" + x.toString ());
		}
	}
	...
	public void flushAll ()
	{
		...
		synchronized (channelList)
		{
			for (Iterator i = channelList.values ().iterator (); i.hasNext ();)
			{
				...
				{
					Log.logFatal (
						"network", "NetworkService.flushAll", "Channel not found: " + channel);
	 ...
	 * @param s The socket.
	 */
	public StreamOrganizer getDefaultStreamOrganizer (Socket s)
		throws IOException
	{
	...
	public void setStreamOrganizer (StreamOrganizer streamOrganizer, double channel)
	{
		...
		try
		{
			synchronized (channelList)
			{
				...
				{
					Log.logFatal (
						"network", "NetworkService.setStreamOrganizer",
		...
		catch (IOException x)
		{
			Log.logFatal (
				"network", "NetworkService.setStreamOrganizer",
				"Unable to set stream organizer for channel " + channel);
		}
	}
}