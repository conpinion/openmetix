 ...
 */
public class NetworkProxyLinkedListManager
	extends BaseObject
	implements IObjectListListener
{
	...
	public void iObjectListEvent (IObjectListEvent event)
	{
		...
		{
			ProxyLinkedListRemoveServerAction action =
				new ProxyLinkedListRemoveServerAction(
					event.getOwnerObject ().getUniqueId (), event.getListAttribute (), newProt);
		...
		((User) AppContext.instance ().getUser ()).putNewObjectsMapping (
			new Long(newUniqueId), new Long(newUniqueId));

		clientTransceiver.addReceiver (channelNumber);

		ProxyLinkedListAddServerAction action =
			new ProxyLinkedListAddServerAction(
				event.getOwnerObject ().getUniqueId (), event.getListAttribute (), newProt);
