 ...
 */
public class NetworkSystemListenerImpl
	extends NetworkSystemAdapter
{
	public void error (
	...
	public void connectionTerminated (NetworkService networkBase, Channel connectedChannel)
	{
		...
		{
			Log.logError (
				"network", "NetworkSystemListenerImpl",
				"Unable to close connection for channel " + connectedChannel.getChannelNumber () +
				" (connection state: " + connectedChannel.getConnectionState () + ")");
		}
		...
		Log.log (
			"network", "NetworkSystemListenerImpl",
			"Lost connection to user '" + user + "' (channel number: " +
			connectedChannel.getChannelNumber () + ")", Log.INFO);

		user.setOnline (false);
		user.setLoggedOutDate (new Date());

		((ShutdownManager) Engine.instance ().getManagerRegistry ().getManager ("shutdown")).shutdown (
			user);
		...
		Log.log (
			"network", "NetworkSystemListenerImpl",
			"Cleaned up connection to user '" + user + "' (channel number: " +
			connectedChannel.getChannelNumber () + ")", Log.INFO);
	}
}