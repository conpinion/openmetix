 ...
 */
public class UserLoginServerAction
	...
	private FrameworkAction checkUserLogin (ClientTransceiver clientTransceiver)
	{
		...
		while (iter.hasNext ())
		{
			...
			if (
				...
				(userSearch.getName ().equals (userName)))
			{
				user = userSearch;
		...
		if ((user == null) || (user.isOnline ()))
		{
			if (user == null)
			{
				return (FrameworkAction) new UserLoginFailureAction(
					UserLoginFailureAction.BAD_USERNAME_OR_PASSWORD);
			}

			if ((user != null) && (user.isOnline ()))
			{
				return (FrameworkAction) new UserLoginFailureAction(
					UserLoginFailureAction.USER_ALREADY_ONLINE);
			}
		}
